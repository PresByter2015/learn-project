...
const FILENAME = '活动';
const arrayDatas = [
  {name: '活动', id: 'type=2&login=true&from=activity&source=activity_zonggonghui'},
...
for (let i = 0; i < arrayDatas.length; i++) {
  const curl = `curl -u dongplus:SRo9pt72PqOI -XPOST -H 'Content-Type: application/json' -d '{"scene":"${arrayDatas[i].id}","page":"pages/transfer/transfer"}' https://open.dongplus.cn/wxmp/qrcode/unlimited`;
  const child = child_process.exec (curl, function (err, stdout, stderr) {
    ...
    fs.appendFile (
      `${FILENAME}.txt`,
      `${arrayDatas[i].name}(${arrayDatas[i].id})：${res}\n`,
      'utf8',
      err => {
        // fs.close('文件.txt', (err) => {
        //   if (err) throw err;
