...
class EditFormChange extends Component {
  static propTypes = {
    ...
    timeChange: React.PropTypes.string,
  ...
  constructor(props) {
    ...
    let obj = {}
    this.props.fieldEdit.map((item, key) => {
      obj['isCheck' + key] = item.isCheck
    })
    this.state = {
      size: 'middle',
      dataSetField: this.props.fieldEdit,
      footer,
      scroll: scroll,
      value: 'Text',
      arr: null,
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.fieldEdit !== nextProps.fieldEdit) {
      let isCheckObj = {}
      nextProps.fieldEdit.map((item, key) => {
        isCheckObj['isCheck' + key] = item.isCheck
      })
      this.setState({
        dataSetField: nextProps.fieldEdit,
  ...
  render() {
    ...
    let { refreshTime, timeChange } = this.props
        ...
        <Form horizontal onSubmit={ this.handleSubmit.bind(this) } className="set">
          ...
          <div className="clearfix">
            <div className="left-form">
              <FormItem {...formItemLayout} label={ intl.formatMessage({ id: 'refresh cycle',
                defaultMessage: '刷新周期' }) }  className="ant-width">
                {
                  getFieldDecorator('refreshTime', { initialValue: timeChange })(
                    <Select
                      onChange={this.onChangeRefreshTime.bind(this)}
...
EditFormChange = Form.create()(EditFormChange)

const mapStateToProps = (state) => {
  let { fieldEdit }  = state.dataSet
  let { refreshTime, dataType } = state.options
